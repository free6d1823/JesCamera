...
int EnumFpsByFrameSize(int fd, v4l2_frmivalenum& fit, CamProperty* pCp, int& n)
{
    ...
    ALOGV("Cam fd=%d - frame size %dx%d, format=%X", fd, fit.width, fit.height, fit.pixel_format);
    return n;
}
...
int CameraManager::Reflesh()
{
    ...
    for (i=0;i<18; i++){
        ...
        if(cap.capabilities & V4L2_CAP_READWRITE){
            ALOGV("Cam%d Support V4L2_CAP_READWRITE", i);
        }
        if(cap.capabilities & V4L2_CAP_STREAMING){
            ALOGV("Cam%d Support V4L2_CAP_STREAMING", i);
        }
        CamProperty cp[100];
...
bool Camera::GetCurrentProperty(CamProperty* pCp)
{
    ...
    pCp->field = (int)fmt.fmt.pix.field;
...
{
    if (m_fd >=0) {
        ALOGE("Camera has been opened before.\n");
        return true;
    }
    char device[32];
...
{
    ALOGV("Start thread camid = %d", GetId());
        ...
        if (-1 == xioctl(m_fd, VIDIOC_DQBUF, &buf)) {
            switch (errno) {
                case EAGAIN:
                    ...
                    continue;
                default:
                    ALOGE("VIDIOC_DQBUF error! %d", errno);
                break;
            }
        }
...
int Camera::GetFrame(void* buffer, int length)
{
    ...
    if(m_pfnFramePostProcess) {
        ...
        if ( length < (int) buf.bytesused){
            ALOGE("Buffer too short, need %d bytes, you allocated %d bytes\n",  buf.bytesused, length);
            return -2;
        };
